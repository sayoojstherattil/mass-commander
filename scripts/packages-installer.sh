...
ls /var/cache/apt/archives > /root/mass-commander/runtime-files/files-in-archive-old

echo -ne "enter names of packages one by one: "

while read package_name; do
	echo "$package_name" >> /root/mass-commander/runtime-files/packages-to-install
done

apt-get install --download-only $(cat /root/mass-commander/runtime-files/packages-to-install) -y

#current files_in_archive_fetcher
ls /var/cache/apt/archives > /root/mass-commander/runtime-files/files-in-archive-new

#new_files_in_archive_identifier
cat /root/mass-commander/runtime-files/files-in-archive-new | grep -v -f /root/mass-commander/runtime-files/files-in-archive-old > /root/mass-commander/runtime-files/fetched-deb-files
