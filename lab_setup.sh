...
export files_for_client_loc="$working_dir/files_for_client"
export files_for_client_tarball_loc="$working_dir/files_for_client.tar.xz"
...
export port_no_for_exec="55555"
export port_no_for_file_transfer="66666"
...
ssh_keys_generator() {
	...
	cp $key_for_accessing_client_machines $files_for_client_loc
...
server_setup() {
	...
	mv /root/server-side /root/mass-commander
...
clients_setup() {
	cp -r $mass_commander_client_dir_loc $files_for_client_loc
	tar -cvJf $files_for_client_tarball_loc $files_for_client_loc
	...
	prompt "enter the command in all clients: nc -lp $port_no_for_exec"

	while read client_ip_address; do
		cat $netcat_file_loc | nc -q 1 $client_ip_address $port_no_for_exec
		cat $tarball_for_clients_loc | nc -q 1 $client_ip_address $port_no_for_file_transfer

		scp $key_for_accessing_sftp_server_loc -o StrictHostKeyChecking=no $client_ip_address:.ssh
	done
}
