...
running_users_finder() {
	...
	cat $runtime_files_dir/running-users-details | awk -F' ' '{print $1}' > $runtime_files_dir/running-usernames
}
...
sftp -o StrictHostKeyChecking=no $sftp_username@$sftp_server_ip <<< $"get $sftp_directory/commands-to-run-of-client"
cd -
